...
knn_model = KNeighborsClassifier(n_neighbors=5, metric='euclidean')  # Set number of neighbors and distance metric
...
# Wrap `enumerate` around `tqdm` to properly show progress
for i, sample in enumerate(tqdm(X_test_scaled, desc="Predicting", unit="sample")):
    pred = knn_model.predict([sample])  # Predict one sample at a time
